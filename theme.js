(function () {
  ...
  const previewKey = "aiEngineerResumeDataDraft";
  const themeKey = "aiEngineerResumeTheme";
  const channelName = "ai-engineer-resume-data";
...
self.onmessage = async function (event) {
              ...
              ${data.jobs.map(function (job) {
                return `
                  <tr class="job-row">
                    ...
                    <td colspan="2">${escapeHtml(job.detail)}${jobLinks(job)}</td>
                  </tr>
                `;
              }).join("")}
  ...
  function metrics(data) {
    ...
    return [
      { value: String(data.jobs.length || 7), label: "公開 GitHub 作品" },
      { value: "5+", label: "AI 協作工具" },
      { value: "HTML/JS", label: "主要開發棧" },
      { value: "GitHub Pages", label: "靜態部署經驗" }
